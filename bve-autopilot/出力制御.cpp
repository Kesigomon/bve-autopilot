...
namespace autopilot
{
    ...
    namespace
    {
        ...
        自動制動自然数ノッチ 緩め制動ノッチ(const 共通状態 &状態)
        {
            ...
            // 緩め制動ノッチが作用し始める頃の速度を推測する
            速度 = std::max(速度 - 減速度 * 状態.制動().反応時間(), 0.0_mps);

            // 速度に基づいて許容する最大の減速度を求める。
            // 強い減速度を短い時間だけ作用させるのは乗り心地が悪いので、
            // 強い減速度で減速する場合にはブレーキをかける時間も長くしたい。
            // 速度は減速度と時間の積なので、速度の平方根を取ると期待する
            // 許容減速度と想定される停止までの時間に比例する値が得られる。
            mps2 許容減速度 = mps2(0.8 * std::sqrt(速度.value));

            mps2 勾配加速度 = 状態.車両勾配加速度();
            自動制動自然数ノッチ 緩めノッチ =
                状態.制動().自動ノッチ(許容減速度 + 勾配加速度).ceil();

            // 既に許容減速度よりも強い減速度がかかっている場合は、
            // その減速度を維持するために緩めノッチを強める
            if (許容減速度 < 減速度) {
                自動制動自然数ノッチ 前回ノッチ =
                    状態.制動().自動ノッチ丸め(
                        状態.制動().自動ノッチ(状態.前回制動指令()));
                緩めノッチ = std::max(緩めノッチ, 前回ノッチ);
            }
            return 緩めノッチ;
        }
