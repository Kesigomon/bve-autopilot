...
namespace autopilot
{
    ...
    namespace
    {
        ...
        std::pair<制動指令, 出力元> 出力制動指令を選択(
            手動制動自然数ノッチ 手動ノッチ, 自動制動自然数ノッチ 自動ノッチ,
            出力元 自動ノッチ出力元, const 制動特性 &制動)
        {
            ...
            if (手動 > 自動) {
                ...
                return {制動.指令(自動ノッチ), 自動ノッチ出力元};
            }
        }

        std::pair<int, 出力元> 出力力行指令を選択(
            int 手動ノッチ, int 自動ノッチ, 出力元 自動力行元)
        {
            if (手動ノッチ > 自動ノッチ) {
                return {手動ノッチ, 出力元::手動};
            }
        ...
        const ATS_VEHICLESTATE &状態, int *出力値, int *音声状態)
    {
        ...
        auto [自動ノッチ, 自動出力元] = 自動指令を選択();
        auto [制動指令, 制動元] = 出力制動指令を選択(
            _状態.入力制動ノッチ(), 自動ノッチ.制動成分(), 自動出力元,
            _状態.制動());

        ATS_HANDLES ハンドル位置{};
        ハンドル位置.Brake = 制動指令.value;
        ハンドル位置.Reverser = _状態.入力逆転器ノッチ();
        ハンドル位置.ConstantSpeed = ATS_CONSTANTSPEED_CONTINUE;

        出力元 力行元;
        if (_状態.入力力行ノッチ() >= 0) {
            std::tie(ハンドル位置.Power, 力行元) = 出力力行指令を選択(
                変換済入力力行ノッチ(_状態),
                static_cast<int>(自動ノッチ.力行成分().value), 自動出力元);
        }
        else {
    ...
    std::pair<自動制御指令, 出力元> Main::自動指令を選択() const
    {
        if (!tasc有効() || ato一時停止中()) {
            return {自動制御指令{}, 出力元::手動};
        }

        自動制御指令 指令 = _状態.最大力行ノッチ();
        出力元 選択 = 出力元::手動;
        if (ato有効()) {
            自動制御指令 ato指令 = _ato.出力ノッチ();
            if (ato指令 <= 指令) {
                指令 = ato指令;
                選択 = 出力元::ato;
            }
        ...
        自動制御指令 tasc指令 = _tasc.出力ノッチ();
        if (tasc指令 <= 指令) {
            if (tasc指令 < 自動制御指令{} || インチング中()) {
                指令 = tasc指令;
                選択 = 出力元::tasc;
            }
            else if (ato有効()) {
                指令 = tasc指令;
                // TASC は制動しかしないことになっているので
                // 惰行・力行指令は ATO によるものと見せかける
                選択 = 出力元::ato;
            }
            else {
                指令 = 自動制御指令{};
        ...
        if (_状態.入力制動ノッチ() > 手動制動自然数ノッチ{0} ||
            _状態.入力逆転器ノッチ() <= 0)
        {
            指令 = std::min(指令, 自動制御指令{力行ノッチ{0}});
        }
